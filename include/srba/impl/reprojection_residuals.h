	...
	) const
{
	...
	//-------------------------------------------------------------
	// Estimates histogram and save it to hist_aux.png (SUPER SLOW)
	//-------------------------------------------------------------
	// Sort residual
	...
	double stdv = 1.4826f * residual_aux(nObs);

	// Histogram definitions
	int N_bins = 1000, hist_idx = 1, hist_max_bins, hist_samples;
	double hist_min = 0.f, hist_max = residual_sort(2*nObs-1);
	Eigen::VectorXi hist = Eigen::VectorXi::Zero(N_bins);
	hist_max = 2.0;
	// Estimates histogram
	//std::cout << std::endl << "#Observations: " << nObs << std::endl << std::endl;
	for(int i = 0; i < 2*nObs; i++)
	{
		//std::cout << residual_sort(i) << " " ;
		if( residual_sort(i) <= ( double(hist_idx)*hist_max/double(N_bins) ) )
		{
			hist(hist_idx-1)++;
			hist_samples++;
		}
	...
	ofstream hist_file(filename.c_str());
	hist_file << 0.0 << " " << 0.0 << " " << N_bins << " " << hist_max << " " << hist_samples << endl;
		...
		)
{
	...
	if( alpha+1.f > 0.f && theta > 0.f )//&& theta < 2.f )
	{
		...
		for (size_t i=0;i<nObs;i++)
		{
			...
			//std::cout << residual_(2*i) << " " << weight(0) << "\t\t" << residual_(2*i+1) << " " << weight(1) << std::endl ;
	...
	else
	{
		...
		// TODO: include use_robust_kernel (cauchy vs huber) to automatically select between both options in the config file)
		...
		)
{
	...
	for (size_t i=0;i<nObs;i++)
	{
		...
		/*if( residual_(2*i) == 1 || residual_(2*i+1) == 1 )
		{
			std::cout << std::endl;
			std::cout << "z_obs = " << real_obs.transpose() << "\t pred_obs = " << (real_obs-delta).transpose() << "\t delta = " << delta.transpose() << std::endl;
		}*/
	...
	// Save into a txt file
	ofstream hist_file(filename.c_str());
	hist_file << alpha + 1.f << " " << theta << " " << N_bins << " " << hist_max << " " << hist_samples << endl;
	for( int i = 0; i < N_bins; i++)
	{
		hist_file << hist(i) << endl;
	}
	...
	if( alpha+1.f > 0.f && theta > 0.f )//&& theta < 2.f )
	{
		...
		std::cout << std::endl << "Gamma parameters: \t alpha = "  << alpha+1.f << " \t theta = " << theta << " \t mean = " << mean << " \t var = " << var;
