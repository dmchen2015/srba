    ...
    ) const
{
    ...
    double mean  = res / std::max(1e-11,(double)(samples));
    double theta = var / std::max(1e-11,mean);
    double alpha = var / std::max(1e-11,theta*theta) - 1.f;  // Since we only use it to estimate weights, we estimate alpha-1

    // Weights assignation
    if( alpha > 0.f && theta > 0.f )
    {
        for (size_t i=0;i<nObs;i++)
        {
            ...
            //std::cout << std::endl << weight(0) << "\t" << weight(1) << "\t" << alpha << "\t" << theta << std::endl << std::endl;
